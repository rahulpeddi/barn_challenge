...
struct Node
{
    ...
    std::vector<quadtree::Node*> children;
...
public:
    Quadtree(const Box<Float>& box, const GetBox& getBox = GetBox(),
        ...
        mBox(box), mRoot(std::make_unique<Node>()), mGetBox(getBox), mEqual(equal), count(0)
    {
    ...
    {
        bool success = remove(mRoot.get(), mBox, value);
...
private:
    ...
    void query(Node* node, const Box<Float>& box, const Box<Float>& queryBox, std::vector<T>& values) const
    {
        ...
        if(!queryBox.intersects(box)){
            ROS_INFO("(%.2f,%.2f)\t(%.2f,%.2f)", queryBox.left, queryBox.top,
                queryBox.getRight(), queryBox.getBottom());
        }
